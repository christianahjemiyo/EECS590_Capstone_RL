...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--base-config", default="../V2/v2_pipeline/configs/mdp_sim_mimic.json")
...
def build_mdp_for_candidate(cfg_path: Path, outdir: Path) -> Path:
    ...
    env["PYTHONPATH"] = "src"
    subprocess.run(
        [sys.executable, str(ROOT.parent / "V2" / "scripts" / "build_mdp.py"), "--config", str(cfg_path), "--outdir", str(outdir)],
        cwd=ROOT,
        check=True,
        env=env,
    )
