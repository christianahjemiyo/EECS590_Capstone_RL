...
def resolve_mdp_path(user_path: str | None) -> Path:
    ...
    if user_path:
        ...
        # Keep compatibility with older path spellings while preferring the
        # versioned V2 layout now used by the repository.
        ...
        if requested_str.startswith("outputs/v2_outputs/"):
            candidates.append(Path("..") / "V2" / Path(requested_str))
        if requested_str.startswith("outputs/mdp/"):
            candidates.append(Path("..") / "V1" / Path(requested_str))
    else:
        candidates.extend(
            [
                Path("../V2/outputs/v2_outputs/mdp/mdp.npz"),
                Path("../V1/outputs/mdp/mdp.npz"),
            ]
        )
...
def maybe_load_saved_policies() -> tuple[dict[str, dict[str, int]], list[str]]:
    ...
    policy_candidates = {
        "v2_tabular_q_policy": ROOT.parent / "V2" / "outputs" / "v2_outputs" / "rl" / "policy.json",
        "v2_dqn_policy": ROOT.parent / "V2" / "outputs" / "v2_outputs" / "rl_dqn" / "policy.json",
        "v2_double_q_policy": ROOT.parent / "V2" / "outputs" / "v2_outputs" / "rl_double_q" / "policy.json",
    }
