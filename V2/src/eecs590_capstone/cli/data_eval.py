...
def main() -> None:
    ...
    parser.add_argument("--outdir", type=str, default="outputs/data_eval")
    ...
    policy = args.policy
    if args.policy_path:
        policy_cfg = load_json(Path(args.policy_path))
        policy = policy_cfg.get("policy", policy)

    results = run_episodes(env, policy, args.episodes, args.seed)
