...
def main() -> None:
    ...
    parser.add_argument("--out-root", default="outputs/v2_outputs")
    ...
    if not args.skip_dqn:
        dqn_cmd = [
            py,
            "v2_pipeline/scripts/train_dqn.py",
            "--mdp",
            mdp,
            "--outdir",
            str(out_root / "rl_dqn"),
    ...
    run_cmd([py, "v2_pipeline/scripts/plot_v2_results.py"], cwd=repo_root, env=env)
    run_cmd([py, "v2_pipeline/scripts/write_v2_interpretation.py"], cwd=repo_root, env=env)
    run_cmd(
        [
            py,
            "v2_pipeline/scripts/plot_v2_saliency.py",
            "--data",
            data,
            "--outdir",
    ...
    run_cmd(
        [
            py,
            "v2_pipeline/scripts/plot_v2_nn_saliency.py",
            "--outdir",
            str(out_root / "figures"),
        ],
        cwd=repo_root,
    ...
    run_cmd(
        [
            py,
            "v2_pipeline/scripts/plot_v2_special_visuals.py",
            "--outdir",
            str(out_root / "figures"),
        ],
        cwd=repo_root,
    ...
    if not args.skip_offline:
        offline_cmd = [
            py,
            "v2_pipeline/scripts/offline_rl_benchmark.py",
            "--mdp",
            mdp,
            "--outdir",
