...
def main() -> None:
    out_path = Path("outputs/v2_outputs/INTERPRETATION.md")
    out_path.parent.mkdir(parents=True, exist_ok=True)

    pi = maybe_eval(Path("outputs/v2_outputs/mdp/policy_iter_eval.json"))
    vi = maybe_eval(Path("outputs/v2_outputs/mdp/value_iter_eval.json"))
    td0 = maybe_eval(Path("outputs/v2_outputs/rl_td0/eval_results.json"))
    sarsa = maybe_eval(Path("outputs/v2_outputs/rl_sarsa/eval_results.json"))
    ql = maybe_eval(Path("outputs/v2_outputs/rl/eval_results.json"))
    dql = maybe_eval(Path("outputs/v2_outputs/rl_double_q/eval_results.json"))
    dqn = maybe_eval(Path("outputs/v2_outputs/rl_dqn/eval_results.json"))
    ...
    lines.append("- `outputs/v2_outputs/mdp/*_value_bar.png`: how valuable each risk-state is under the learned plan.")
    lines.append("- `outputs/v2_outputs/mdp/*_policy_bar.png`: which action each state is assigned.")
    lines.append("- `outputs/v2_outputs/figures/rl_learning_curve.png`: how quickly RL improves with more episodes.")
    lines.append("- `outputs/v2_outputs/figures/algo_avg_return_comparison.png`: side-by-side comparison of final performance.")
    lines.append("")
